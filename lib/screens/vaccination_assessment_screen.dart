...
class _VaccinationAssessmentScreenState
  ...
  Future<void> _loadFirstVisitReview() async {
    try {
      final review = await _repository.getFirstVisitReview(widget.child.id);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('First-visit review could not be loaded: $error'),
        ),
      );
    }
  }
  ...
  Future<void> _recordFirstVisitDecision(bool hasDocuments) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('First-visit review could not be saved: $error'),
        ),
      );
  ...
  String _statusTitle(VaccinationAssessmentStatus status) {
    switch (status) {
  ...
  IconData _statusIcon(VaccinationAssessmentStatus status) {
    switch (status) {
      case VaccinationAssessmentStatus.firstVaccination:
  ...
  Color _statusColor(BuildContext context, VaccinationAssessmentStatus status) {
  ...
  bool _canContinue(VaccinationAssessment assessment) {
    ...
    if (assessment.status == VaccinationAssessmentStatus.firstVaccination) {
      if (_reviewLoading) {
        return false;
      }
  ...
  Future<void> _continueToInventory(VaccinationAssessment assessment) async {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<VaccinationAssessment>(
        ...
        builder: (context, snapshot) {
            ...
            return const Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 16),
                  Text('Checking vaccination history...'),
                ],
              ),
            );
          }
          ...
          return _buildAssessment(context, snapshot.data!);
        },
      ),
    );
  }
  ...
  Widget _buildAssessment(
    ...
    final color = _statusColor(context, assessment.status);

    final canContinue = _canContinue(assessment);

    final isFirstVaccination =
        assessment.status == VaccinationAssessmentStatus.firstVaccination;
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            _SectionCard(
              ...
              child: assessment.history.isEmpty
                  ? const Padding(
                      ...
                      child: Text('No previous vaccination records found.'),
                    )
                  : Column(
                      children: assessment.history
                          .map(
                            (record) => _VaccinationHistoryItem(
                              record: record,
            ...
            _SectionCard(
              ...
              child: Column(
                children: assessment.schedule
                    .where(
                      (entry) => entry.status != PnipDoseStatus.notEligible,
                    )
                    .map(
                      (entry) =>
                          _ScheduleItem(entry: entry, formatDate: _formatDate),
                    )
                    .toList(),
              ),
            ),
            ...
            // -----------------------------------------------------------------
            if (assessment.recommendedVaccines.isNotEmpty)
              _SectionCard(
                title: 'Recommended for Assessment',
                icon: Icons.vaccines_outlined,
                color: Theme.of(context).colorScheme.secondary,
                child: Column(
                  children: assessment.recommendedVaccines
                      .map(
                        (vaccine) => Container(
                          width: double.infinity,
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed: canContinue
                    ? () => _continueToInventory(assessment)
                    : null,
            ...
            // -----------------------------------------------------------------
            if (isFirstVaccination &&
                assessment.recommendedDoses.isNotEmpty &&
...
class _ScheduleItem extends StatelessWidget {
  ...
  const _ScheduleItem({required this.entry, required this.formatDate});

  @override
  Widget build(BuildContext context) {
    final (label, color) = switch (entry.status) {
      PnipDoseStatus.completed => ('Completed', StatusColors.completed),
      PnipDoseStatus.due => ('Due today', StatusColors.due),
      PnipDoseStatus.overdue => ('Overdue', StatusColors.overdue),
      PnipDoseStatus.upcoming => ('Upcoming', StatusColors.upcoming),
      PnipDoseStatus.notEligible => ('Not eligible', Colors.grey),
    };
...
class _VaccinationHistoryItem extends StatelessWidget {
