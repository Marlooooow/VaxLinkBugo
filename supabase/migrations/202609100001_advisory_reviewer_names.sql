...
-- Include the reviewing staff member's display name in the existing paginated
-- advisory response. Authorization, filtering, and pagination remain unchanged.
    ...
    'items', coalesce((
      select jsonb_agg(
        ...
        || jsonb_build_object(
          ...
          'reviewed_by_profile', case when reviewer.id is null then null else
            jsonb_build_object('full_name', reviewer.full_name) end
      ...
      left join public.profiles reviewer on reviewer.id = page.reviewed_by
    ), '[]'::jsonb),
