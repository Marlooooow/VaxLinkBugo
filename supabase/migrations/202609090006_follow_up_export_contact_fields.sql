...
as $$
  ...
  ranked_children as (
    select r.child_id, min(r.created_at) as first_reminder_created_at
  ...
  select jsonb_build_object(
    'items', coalesce(
      (
        select jsonb_agg(
          pr.item order by pr.first_reminder_created_at,
            pr.reminder_created_at, pr.reminder_id
        )
        from page_rows pr
      ),
      '[]'::jsonb
    ),
    'has_more', greatest(page_offset, 0) +
      (select selected_children from selected) <
      (select total_children from totals),
    'next_offset', greatest(page_offset, 0) +
      (select selected_children from selected)
  );
$$;
