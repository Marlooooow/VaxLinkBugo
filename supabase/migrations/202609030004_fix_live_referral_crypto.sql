...
declare
  ...
  item_code text;
begin
  select c.* into child_record from public.children c
  ...
  foreach vaccine_id in array target_vaccine_ids loop
    select coalesce(max(vr.dose_number), 0) + 1 into dose_number from public.vaccination_records vr
    where vr.child_id = child_record.id and vr.vaccine_id = vaccine_id and vr.status <> 'voided';
    select * into rule_record from public.pnip_schedule_rules r
    where r.vaccine_id = vaccine_id and r.dose_number = dose_number and r.active = true
      and r.effective_from <= current_date and (r.effective_to is null or r.effective_to >= current_date)
    order by r.effective_from desc limit 1;
    if rule_record.id is null then raise exception 'No active PNIP schedule rule exists for % dose %.', vaccine_id, dose_number; end if;
    ...
    insert into public.referral_items (id, referral_code, referral_group_id, vaccine_id, dose_number, status)
